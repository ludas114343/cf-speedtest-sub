...
PRIORITY_ORDER = [
    ...
    "SYD", "MEL", "AKL", "YYZ", "YVR", "ICN", "NRT", "KIX", "SIN", "HKG",
    "TPE", "LAX", "SJC", "IAD"
]
...
FALLBACK_DATA = [
    ...
    ("104.18.110.60", 0.0, 45.1, 26.1, "ICN"),
    ("104.16.120.40", 0.0, 61.2, 31.2, "NRT"),
    ("104.18.125.70", 0.0, 63.4, 29.5, "KIX"),
    ("104.16.130.50", 0.0, 65.0, 27.8, "SIN"),
    ("104.18.135.80", 0.0, 67.2, 25.3, "SIN"),
    ("104.16.140.60", 0.0, 35.4, 34.2, "HKG"),
    ("104.18.145.90", 0.0, 37.2, 32.8, "HKG"),
    ("104.16.150.70", 0.0, 135.0, 24.5, "LAX"),
    ("104.18.155.15", 0.0, 138.0, 23.1, "SJC"),
    ("104.16.160.80", 0.0, 168.0, 21.0, "IAD"),
...
def select_nodes(records):
    ...
    for c in PRIORITY_ORDER:
        if c in grouped and grouped[c]:
            quota = 2 if c in ["MFM", "ZRH", "LUX", "ICN", "NRT", "SIN", "HKG"] else 1
...
def write_outputs(nodes, txt_file, md_file):
    ...
    md = [
        ...
        f"> 🌍 **节点总数**: **{len(nodes)}** 个优选节点（覆盖澳门免广告、瑞士中立、卢森堡、法国、德国、爱尔兰、澳大利亚、新西兰、加拿大、韩国、日本、新加坡等）  ",
        "",
        "---",
        "",
